    ...
    static async replace(project:ProjectModel):Promise<string>{
        return new Promise(async (resolve,reject)=>{
            ...
            .then(async (copiedText) => {
                ...
                //here
                await ClipboardUtils.setText(this.replaceAddOutParameter(this.replaceOracleDbType(this.replaceParenthesis(copiedText)))).then((message) => {
                    resolve(message);
                }).catch((err)=>{
                    reject(err);
                });
    ...
    static replaceAddOutParameter(text: string): string {
        ...
        const replacedLines = lines.map((line) => {
          if (line.includes('db.AddOutParameter(comando,')) {
            const replacedLine = line.replace('db.AddOutParameter(comando,', 'comando.Parameters.Add(');
    ...
    static replaceParenthesis(text: string): string {
        ...
        const replacedLines = lines.map((line) => {
          if (line.includes(searchString) && line.trim().endsWith(');')) {
            ...
            const lineWithoutParenthesis = line.slice(0, lastIndex) + ';' + line.slice(lastIndex + 1, -2);
    ...
    static replaceOracleDbType(text: string): string {
        ...
        const replacedLines = lines.map((line) => {
          if (line.includes('OracleDbType.Boolean')) {
            ...
            const replacedLine = line.replace('OracleDbType.Boolean', 'OracleDbType.Int32');
            if (isTernary) {
              ...
              const newTernary = afterTernary.replace(/true/g, '1').replace(/false/g, '0');
